...
- [x] T013 [P] Create root `docker-compose.yml` — GHCR+Pull pattern (no local build on VPS): service `backend`: `image: ghcr.io/${GITHUB_REPOSITORY_OWNER}/pl-jobhunter/backend:latest`, env_file `./apps/backend/.env`, volume `./apps/backend/wallet:/app/apps/backend/wallet:ro`, port `3000:3000`, depends_on `ollama`; service `ollama`: `image: ollama/ollama:0.6.10`, named volume `ollama_models:/root/.ollama`, port `11434:11434`
- [x] T014 [P] Create `.github/workflows/ci-cd.yml` — trigger on push/PR to `dev` + `main`; job `ci`: `ubuntu-latest`, Node 22, pnpm install, security audit, `tsc --noEmit` for shared + backend + frontend, `vitest run`; job `build-and-push` (push to `main` only): docker buildx build + push to GHCR with `<sha>` and `latest` tags, registry cache; job `deploy` (after build-and-push): SSH → VPS → `docker compose pull backend && docker compose up -d backend && docker image prune -f`
...
- [x] T022 [P] [US1] Create `apps/frontend/src/api/client.ts` — typed fetch wrapper reading `VITE_API_TOKEN` (auth header) and `VITE_API_BASE_URL` (base URL, default `''`) from `import.meta.env`; exports `getJobs(): Promise<JobWithAnalysis[]>`, `patchJobStatus(id, status): Promise<void>`, `getProfile(): Promise<UserProfile | null>`, `putProfile(data): Promise<UserProfile>`
...
- [x] T054 Run `docker build -t pl-jobhunter-backend ./apps/backend` — confirm multi-stage build succeeds (NOTE: build context must be repo root: `docker build -f apps/backend/Dockerfile -t pl-jobhunter-backend .`)
- [x] T055 Run `docker compose up -d` from repo root — confirm backend + ollama containers start; `curl localhost:3000/health` returns 200 with valid token
